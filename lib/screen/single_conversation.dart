...
class SingleConversation extends StatefulWidget {
  ...
  const SingleConversation({Key? key, required this.name, required this.id});
...
class _SingleConversationState extends State<SingleConversation> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: Text("Create New Conversation"),
        onPressed: () {
          Navigator.of(context).push(MaterialPageRoute(
              builder: (context) =>
                  NewConversation(name: widget.name, userId: widget.id)));
        },
      ),
      body: Column(
        children: [
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    return ListTile(
                      leading: CircleAvatar(
                        child: Text(snapshot.data![index]["participants"][0]
                        ["full_name"][0]),
                      ),
                      title: Text(
                      ...
                      subtitle: Text(
                        snapshot.data![index]["last_authored_message"] ?? "",
                        maxLines: 2,
                      ...
                      onTap: () => _openChat(context, snapshot.data![index]['id'],
  ...
  void _openChat(BuildContext context, int id, String subject) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => ChatDetails(id: id, subject: subject),
      ),
    );
  }
}
